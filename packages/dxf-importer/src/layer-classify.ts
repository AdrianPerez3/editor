...
const RULES: Rule[] = [
  ...
  { concept: 'door', pattern: /^(A[-_]?DOOR|M[-_]?DOORS?|TUER|TUR|PORTE|PUERTA|PORTA|DOA|TOBIRA)\b/i },
  // Windows / glazing
  { concept: 'window', pattern: /^(A[-_]?GLAZ|A[-_]?WIN|A[-_]?WINDOW|WINDOW|FENSTER|FENETRE|VENTANA|FINESTRA|MADO)\b/i },
  // Stairs (specific A-FLOR-STRS before generic FLOR)
  { concept: 'stair', pattern: /^(A[-_]?FLOR[-_]?STRS|STAIR|TREPPE|ESCALIER|ESCALERA|SCALA|KAIDAN)\b/i },
  // Floors / slabs
  { concept: 'floor', pattern: /^(A[-_]?FLOR|FLOOR|BODEN|SOL|PLANCHER|SUELO|PISO|PAVIMENTO|YUKA)\b/i },
  // Roof
  { concept: 'roof', pattern: /^(A[-_]?ROOF|ROOF|DACH|TOITURE|TOIT|CUBIERTA|TETTO|YANE)\b/i },
  // Columns
  { concept: 'column', pattern: /^(A[-_]?COLS|COLUMN|STUETZE|STUTZE|POTEAU|PILAR|COLUMNA|PILASTRO|HASHIRA)\b/i },
  // Rooms / areas
  { concept: 'room', pattern: /^(A[-_]?AREA|ROOM|ZONE|RAUM|PIECE|HABITACION|LOCAL|LOCALE|HEYA|SHITSU)\b/i },
  // Annotations / labels
  { concept: 'label', pattern: /^(A[-_]?ANNO[-_]?TEXT|A[-_]?AREA[-_]?IDEN|A[-_]?ANNO[-_]?IDEN|TEXT|LABEL|TITLE)\b/i },
  // Dimensions
  { concept: 'dimension', pattern: /^(A[-_]?ANNO[-_]?DIMS|DIM[-_]?|DIMENSION|MASS)\b/i },
  // Walls (least specific so doors/windows/stairs are checked first)
  { concept: 'wall', pattern: /^(A[-_]?WALL|WALL|WAND|MUR|MURO|PARETE|TABIQUE|CLOISON|KABE)\b/i },
];
